...
def NoSpecMath(RHS):
	list_specMath = 'log ln exp atan asin acos tan sin cos'.split()
	condition=False
	for specMath in list_specMath:
		if specMath in RHS:
			condition=True
			break
	return condition, specMath
...
def ReverseEqn(str1, dict_reverse, child_var, list_mathOperations):
	...
	SpecMath_condition, specMath = NoSpecMath(RHS)
	if SpecMath_condition:
		reverse_eqn = f'Instance of "{specMath}" inside equation. ***This script is not set up for this'
		return reverse_eqn
