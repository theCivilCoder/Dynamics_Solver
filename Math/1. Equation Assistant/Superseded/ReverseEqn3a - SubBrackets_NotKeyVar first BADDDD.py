...
def SubBrackets_NonKeyVar(RHS, list_mathOperations, dict_bracketEqns_NotKeyVar, child_var):
	InBracketTest=False
	idx_test=0
	...
	#leaves function if the key child_var is in the equation as this is dealt with a separate function
	if child_var in bracket_terms:
		InBracketTest
		return RHS, dict_bracketEqns_NotKeyVar, InBracketTest	
	# print(f'bracket_terms is {bracket_terms}')
...
def ReverseEqn(str1, dict_reverse, child_var, list_mathOperations):
	...
	RHS_i_beforeBracketsNK = RHS
	dict_bracketEqns_NotKeyVar={}
	
	InBracketTest = True
	testnum = 1
	print(f'\n{"*"*5}Check # {testnum}:')
	RHS_i, dict_bracketEqns_NotKey,InBracketTest =SubBrackets_NonKeyVar(RHS, list_mathOperations, dict_bracketEqns_NotKeyVar)
	...
	testnum = 1
	print(f'\n{"*"*5}Check # {testnum}:')
	RHS_i, dict_bracketEqns,InBracketTest = InBrackets(RHS_i, child_var, list_mathOperations, dict_bracketEqns)
